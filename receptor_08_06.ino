#define PWM_CH_VAL_MIN  0.920   //representa un 0 lógico en el canal
#define PWM_CH_VAL_MAX  2.120   //representa un 1 lógico en el canal
#define PWM_PRE_PULSE   0.400   //tiempo entre cada pulso (cada canal)
#define PWM_SEPARATION  0.500   //tiempo entre mensajes
...
void loop() {
 ...
 if (radio.available()){ 
     ...
     for(size_t i=0; i<NUMBER_OF_CHANNELS; i++){
        ...
        delay(PWM_PRE_PULSE);                    //estuvo en high lo que cada channel dice que debe estar, luego esta en low lo que vale PWM_PRE_PULSE
     }
 }
 else{
     Serial.println("No hay datos de radio disponibles");
 }
 delay(PWM_SEPARATION);
}
...
void pwm_read (unsigned char * arr, double * arr_double){
  ...
  for(i=0; i<NUMBER_OF_CHANNELS; i++){
    arr_double[i]=convert_bit_to_usec(arr[i]);
  }
